...
import { Check, Zap, Crown, Users, ShieldCheck, Star } from 'lucide-react';
  ...
  const plans = [
    {
      ...
      features: [
        'Até 10 notas fiscais',
        'IA Processadora Básica',
        'Busca e Filtros',
    ...
    },
    {
      ...
      features: [
        'Tudo do Plano Pro',
        'Até 5 Usuários Integrados',
        'Relatórios Consolidados',
        'Dossiê Jurídico Automático',
    ...
    <div className="max-w-6xl mx-auto space-y-12 pb-20">
      <header className="text-center space-y-4">
        <h1 className="text-5xl font-black tracking-tight text-slate-900">
          Escolha seu nível de <span className="text-emerald-600">Proteção</span>
        </h1>
        <p className="text-slate-500 font-medium max-w-2xl mx-auto">
          O Guardião de Notas oferece soluções para cada necessidade. Evolua sua segurança patrimonial hoje mesmo.
        </p>
      </header>
      ...
      <div className="grid gap-8 md:grid-cols-3">
        {plans.map((plan, idx) => (
          ...
          >
            {plan.popular && (
              <div className="absolute -top-4 left-1/2 -translate-x-1/2 z-20 bg-emerald-600 text-white text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-full shadow-lg">
                Mais Escolhido
              </div>
            )}
              ...
              <CardContent className="p-8 pt-4 flex-1 flex flex-col">
                  ...
                  {plan.features.map((feature, fIdx) => (
                    <div key={fIdx} className="flex items-center gap-3">
                      <div className="h-5 w-5 rounded-full bg-emerald-50 flex items-center justify-center shrink-0">
                        <Check className="h-3 w-3 text-emerald-600" strokeWidth={4} />
                      </div>
                ...
                <div className="mt-10">
                  <Button 
                    ...
                    className={`w-full h-14 text-base font-black ${plan.current ? 'opacity-50 pointer-events-none' : ''}`}
                  >
                    {plan.buttonText}
                  </Button>
                </div>
      ...
      <div className="p-10 glass rounded-[40px] flex flex-col md:flex-row items-center justify-between gap-8">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h3 className="text-xl font-bold text-slate-900 text-left">Garantia Guardião</h3>
            <p className="text-sm text-slate-500 font-medium text-left">Satisfação garantida ou seu dinheiro de volta em até 7 dias.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="ml-2 font-black text-slate-900">4.9/5</span>
        </div>
      </div>
    </div>
  );
