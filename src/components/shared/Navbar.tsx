...
import { Plus, LogOut, LayoutDashboard, User, Sparkles, Crown, Bell, X, Check, BarChart3, Users, ShieldCheck, Wrench, ChevronDown, Plane, History, ShieldBan, FolderOpen } from 'lucide-react';
      ...
      <div className="glass rounded-2xl px-4 py-3 flex items-center justify-between shadow-2xl shadow-emerald-500/10 dark:shadow-emerald-900/20 transition-all duration-500">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <div className="hidden lg:flex items-center gap-1">
            <Link href="/dashboard"><Button variant="ghost" size="sm" className="gap-2 font-bold text-slate-600 dark:text-slate-300"><LayoutDashboard className="h-4 w-4 text-emerald-600" /> Painel</Button></Link>
            <Link href="/vault"><Button variant="ghost" size="sm" className="gap-2 font-bold text-slate-600 dark:text-slate-300"><ShieldCheck className="h-4 w-4 text-emerald-600" /> Cofre</Button></Link>
            <Link href="/analytics"><Button variant="ghost" size="sm" className="gap-2 font-bold text-slate-600 dark:text-slate-300"><BarChart3 className="h-4 w-4 text-emerald-600" /> Análises</Button></Link>
            
            {/* Dropdown de Ferramentas Avançadas */}
              ...
              <AnimatePresence>
                {showMoreMenu && (
                  <motion.div 
                    ...
                    className="absolute right-0 mt-4 w-64 glass rounded-3xl shadow-2xl border border-teal-50 dark:border-white/5 overflow-hidden z-50 p-2 space-y-1"
                  >
                    <MenuLink href="/maintenance" icon={<Wrench className="h-4 w-4" />} title="Revisões" desc="Agenda de manutenção" onClick={() => setShowMoreMenu(false)} />
                    <MenuLink href="/family" icon={<Users className="h-4 w-4" />} title="Família" desc="Gestão compartilhada" onClick={() => setShowMoreMenu(false)} />
                    <MenuLink href="/audit" icon={<History className="h-4 w-4" />} title="Auditoria" desc="Logs de segurança" onClick={() => setShowMoreMenu(false)} />
                    <MenuLink href="/travel-check" icon={<Plane className="h-4 w-4" />} title="Modo Viagem" desc="Declaração de mala" onClick={() => setShowMoreMenu(false)} />
                    <MenuLink href="/notifications/recalls" icon={<ShieldBan className="h-4 w-4" />} title="Recall Central" desc="Alertas de fábrica" onClick={() => setShowMoreMenu(false)} />
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
          ...
          <Link href="/plans" className="hidden sm:block"><Button size="sm" className="h-10 px-4 rounded-xl font-black text-[10px] uppercase tracking-widest bg-emerald-600 text-white">Upgrade</Button></Link>
...
function MenuLink({ href, icon, title, desc, onClick }: { href: string, icon: any, title: string, desc: string, onClick: () => void }) {
  return (
    <Link href={href} onClick={onClick} className="flex items-center gap-4 p-3 rounded-2xl hover:bg-emerald-50 dark:hover:bg-white/5 group transition-all">
